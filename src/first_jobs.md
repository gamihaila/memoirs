...
Windows 3.1, in 1991. I learned how to write programs for Windows from
the Charles Petzold book, Programming Windows. At the time, the
programs were single threaded, and the book taught us to write event
handlers that returned control quickly because otherwise the operating
system would be frozen. These were interactive programs: a user would
push a button on the UI, or select a menu item, something needed to
happen in response, and then the program would wait for the next user
